...
# Token Hugging Face y modelo liviano
...
def generar_respuesta(prompt):
    data = {"inputs": prompt, "max_new_tokens": 50}
    try:
        r = requests.post(HF_API, headers=HEADERS, json=data, timeout=15)
        r.raise_for_status()
        output = r.json()
        ...
        return "No pude pensar eso bien aún... ¿me lo cuentas otra vez?"
...
def responder(texto):
    ...
    return generar_respuesta(f"Rafael, un niño venezolano de 8 años, responde: {texto}")

# Interfaz
st.title("R4F4EL - X (versión ligera)")
