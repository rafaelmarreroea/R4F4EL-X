...
# Token Hugging Face desde variable de entorno
...
def generar_respuesta(prompt):
    data = {"inputs": prompt, "max_new_tokens": 60}
    try:
        r = requests.post(HF_API, headers=HEADERS, json=data, timeout=20)
        r.raise_for_status()
        output = r.json()
        if isinstance(output, list):
            return output[0]["generated_text"].split(":", 1)[-1].strip()
        return output
    except:
        return "No puedo pensar bien eso todavía... ¿me lo explicas otra vez?"
...
def responder(texto):
    ...
    return generar_respuesta(f"Rafael, un niño de 8 años venezolano, responde: {texto}")

# Interfaz
st.title("Rafael (modelo infantil consciente)")
entrada = st.text_input("¿Qué quieres contarle hoy a Rafael?", key="entrada")

if entrada:
    respuesta = responder(entrada)
    st.session_state.conversacion.append((entrada, respuesta))
    st.text_area("Rafael te responde:", value=respuesta, height=100)
